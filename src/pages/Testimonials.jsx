...
const TestimonialCard = ({ position, testimonial, handleMove, cardSize }) => {
  ...
  return (
    <Motion.div
      ...
      style={{ width: cardSize, minHeight: cardSize, height: 'auto' }}
    >
      <img
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (fetched.length) setTestimonialsList(fetched);
      } catch (e) {
    ...
    <Motion.div className="testimonials-page">
      ...
      {testimonialsList.length > 0 ? (
        ...
        <div className="testimonials-empty-state">
          <p>No recommendations yet. Be the first to leave one below! ✨</p>
        </div>
      )}
